...
const AllCourses = () => {
  ...
  useEffect(() => {
    const fetchHandler = async () => {
      ...
      try {
        ...
        setComps2(await GetMyCourses(info._id));
      } catch (error) {
      } finally {
        setLoading(false);
      }
  ...
  // comps[0] !== undefined && console.log(comps[0].students[0]._id);
  ...
  const addRow = async (id) => {
    const response = await AddStudentToCourses(info._id, id);
    response
      ? notify("درس با موفقیت اخذ شد", "success")
      : notify("موفقیت آمیز نبود", "error");
    response && fetchHandler2();
  };
            ...
            <div className="mt-8">
              <div className="flex flex-row-reverse justify-center items-center">
                <img
                  ...
                  alt="logo pic"
                />
                <div className="relative  right-1">لیست دوره ها</div>
              </div>
            </div>
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                      ...
                      <div className="flex items-center flex-row-reverse justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            addRow(idPicker) & setShowAddModal(false)
                          }
                        >
                          بله
                        </button>
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                  <TableHead>
                    <TableRow>
                      ...
                      <StyledTableCell align="center">ظرفیت</StyledTableCell>
                    ...
                    {DATA.currentData().map((row) => (
                      <StyledTableRow key={row._id}>
                        ...
                        <StyledTableCell align="left">
                          {/* {console.log(row._id + "  1")}
                          {comps2.map((myId) => console.log(myId._id))} */}
                          {/* {console.log(comps2.map((myId) => myId._id === row._id))} */}
                          {comps2
                            .map((myId) => myId._id === row._id)
                            .includes(true) ? (
                            <DeleteOutlineIcon
                              sx={{ color: red[600] }}
                            ...
                            <AddCircleOutlineOutlinedIcon
                              ...
                              className="cursor-pointer"
                              onClick={() =>
                                setShowAddModal(true) & setIdPicker(row._id)
                              }
                            />
                          )}
                        </StyledTableCell>
